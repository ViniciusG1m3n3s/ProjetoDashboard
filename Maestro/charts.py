...
def plot_status_pie(df, column_name, custom_colors):
...
def grafico_status_analista(df, column_name, analista_column, custom_colors):
    df_grouped = df.groupby([analista_column, column_name]).size().unstack(column_name)
    fig_status = px.pie(
        names=df_grouped.columns,
        values=df_grouped.values[0],
        color_discrete_sequence=custom_colors
    )
    fig_status.update_traces(
        hovertemplate='Tarefas %{label} = %{value}<extra></extra>',
